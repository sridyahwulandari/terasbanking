<?php

namespace backend\models;

use yii\base\Model;
use yii\data\ActiveDataProvider;
use backend\models\Transfer;

/**
 * TransferSearch represents the model behind the search form of `backend\models\Transfer`.
 */
class TransferSearch extends Transfer
{
    /**
     * {@inheritdoc}
     */
    public function rules()
    {
        return [
            [['id_transfer', 'id_rekening'], 'integer'],
            [['tanggal', 'rekening_tujuan', 'nama_pemilik_rekening', 'jumlah', 'no_referensi_nasabah', 'biaya'], 'safe'],
        ];
    }

    /**
     * {@inheritdoc}
     */
    public function scenarios()
    {
        // bypass scenarios() implementation in the parent class
        return Model::scenarios();
    }

    /**
     * Creates data provider instance with search query applied
     *
     * @param array $params
     *
     * @return ActiveDataProvider
     */
    public function search($params)
    {
        $query = Transfer::find();

        // add conditions that should always apply here

        $dataProvider = new ActiveDataProvider([
            'query' => $query,
        ]);

        $this->load($params);

        if (!$this->validate()) {
            // uncomment the following line if you do not want to return any records when validation fails
            // $query->where('0=1');
            return $dataProvider;
        }

        // grid filtering conditions
        $query->andFilterWhere([
            'id_transfer' => $this->id_transfer,
            'tanggal' => $this->tanggal,
            'id_rekening' => $this->id_rekening,
        ]);

        $query->andFilterWhere(['like', 'rekening_tujuan', $this->rekening_tujuan])
            ->andFilterWhere(['like', 'nama_pemilik_rekening', $this->nama_pemilik_rekening])
            ->andFilterWhere(['like', 'jumlah', $this->jumlah])
            ->andFilterWhere(['like', 'no_referensi_nasabah', $this->no_referensi_nasabah])
            ->andFilterWhere(['like', 'biaya', $this->biaya]);

        return $dataProvider;
    }
}
